...
export default function Stats() {
  ...
  return (
    <section ref={ref} className={`section-pad${vis ? ' reveal visible' : ' reveal'}`}>
      <p className="section-eyebrow">FASTY en números</p>
...
function StatCard({ target, label, fmt, animate }: { target: number | null; label: string; fmt: (n: number) => string; animate: boolean }) {
  ...
  return (
    <div style={{ background: 'var(--dark3)', border: `1px solid ${h ? 'rgba(200,241,53,0.3)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 'var(--radius)', padding: '2rem 1.5rem', position: 'relative', overflow: 'hidden', transition: 'transform 0.25s,border-color 0.25s', transform: h ? 'translateY(-4px)' : 'none' }}
      ...
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(2rem,4vw,2.8rem)', fontWeight: 800, lineHeight: 1, marginBottom: '0.5rem', background: 'linear-gradient(135deg,var(--white),rgba(255,255,255,0.6))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
