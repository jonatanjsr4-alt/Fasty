    ...
    <>
      <nav style={{ position:'fixed',top:0,left:0,right:0,zIndex:500,padding:'1rem 2rem',display:'flex',alignItems:'center',justifyContent:'space-between',backdropFilter:'blur(20px)',background:scrolled?'rgba(10,10,10,0.97)':'rgba(10,10,10,0.7)',borderBottom:'1px solid rgba(255,255,255,0.05)',transition:'background 0.3s' }}>
        ...
        <div style={{ display:'flex',alignItems:'center',gap:'0.8rem' }}>
          ...
          {isLoggedIn
            ? <Link href="/profile" style={{ width:40,height:40,borderRadius:12,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.08)',display:'flex',alignItems:'center',justifyContent:'center',color:'var(--white)' }}><User size={16} /></Link>
      ...
      <div className={`nav-mobile-menu${menuOpen ? ' open' : ''}`}>
        ...
        {isLoggedIn
          ? <Link href="/profile" onClick={() => setMenuOpen(false)} style={{ color:'var(--muted)' }}>Mi perfil</Link>
