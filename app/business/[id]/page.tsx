...
import { Star, Clock, ShoppingBag, ArrowLeft, Plus, Minus, MapPin } from 'lucide-react'
...
type Restaurant = {
  ...
  description: string
  category: string
  image_url: string   // columna real en restaurants
  logo_url: string
  rating: number
...
type Product = {
  ...
  description: string
  price: number
  image: string        // ← columna real en products
  category: string
  available: boolean   // ← columna real en products
}
...
export default function StorePage() {
  ...
  useEffect(() => { if (storeId) fetchData() }, [storeId])
  ...
  async function fetchData() {
    ...
    const { data: prods } = await supabase
      ...
      .eq('available', true)   // ← columna real
      .order('category')
  ...
  function handleAddToCart(product: Product) {
    addToCart({
      ...
      name: product.name,
      price: product.price,
      image: product.image || '',   // ← columna real
      ...
      <main style={{ minHeight: '100vh', background: 'var(--dark)', paddingTop: '4rem' }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 2rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', overflowX: 'auto', paddingBottom: '0.5rem', marginBottom: '1.5rem' }}>
              {categories.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)}
                  style={{ padding: '6px 16px', borderRadius: 100, fontSize: '0.78rem', whiteSpace: 'nowrap', cursor: 'pointer', transition: 'all 0.2s', background: activeCategory === cat ? 'var(--orange)' : 'var(--dark3)', color: activeCategory === cat ? '#fff' : 'var(--muted)', border: `1px solid ${activeCategory === cat ? 'var(--orange)' : 'rgba(255,255,255,0.08)'}` }}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          {filteredProducts.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem', paddingBottom: '8rem' }}>
        ...
        {itemCount > 0 && (
          <div style={{ position: 'fixed', bottom: 24, left: '50%', transform: 'translateX(-50%)', zIndex: 800 }}>
            <button onClick={() => setCartOpen(true)}
              ...
              Ver carrito · <span style={{ opacity: 0.85 }}>${total.toLocaleString()}</span>
            </button>
          </div>
        )}
      </main>
...
function ProductCard({ product, justAdded, onAdd }: { product: Product; justAdded: boolean; onAdd: () => void }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ background: 'var(--dark3)', border: `1px solid ${hovered ? 'rgba(255,80,1,0.25)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 20, overflow: 'hidden', transition: 'transform 0.25s, border-color 0.25s', transform: hovered ? 'translateY(-3px)' : 'none' }}>

      {/* IMAGE — usa product.image */}
      <div style={{ height: 160, background: 'var(--mid)', overflow: 'hidden', position: 'relative' }}>
        {product.image ? (
          <img src={product.image} alt={product.name} style={{ width: '100%', height: '100%', objectFit: 'cover', transition: 'transform 0.4s', transform: hovered ? 'scale(1.05)' : 'scale(1)' }} />
        ) : (
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '0.5rem' }}>
          ...
          {cartItem ? (
            ...
            <button onClick={onAdd}
              style={{ width: 34, height: 34, borderRadius: 10, cursor: 'pointer', border: 'none', background: justAdded ? '#22c55e' : 'var(--orange)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s, transform 0.15s', transform: justAdded ? 'scale(1.2)' : 'scale(1)' }}>
